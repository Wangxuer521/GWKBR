...
@njit
def gibbs_sampling_core(n_iter, burn_in, alpha, beta, y, K, G,
...
def calculate_snp_weights(X_train, id_train):
    np.savetxt('id_train.txt', id_train, fmt='%d')
    
    # Generate reference population individual list
    try:
        with open('id_train.txt', 'r') as r1, \
            open('./example_data/all_genotypes.fam', 'r') as r2, \
    ...
    try:
        ...
        merged = pd.merge(bim_df[['SNP']], assoc_df[['SNP','beta','se']], on='SNP', how='inner')

        merged['SNP'].to_csv('final_snps', index=False, header=False)
    ...
    try:
        ...
        os.system("mv snp_weight.out snp_weight_ref.out")
        
        # Delete the temporary files
        files_to_remove = ['ref_genotypes.bed', 'ref_genotypes.bim', 'ref_genotypes.fam', 'gwas.result', 'final_snps', 'plink.raw', 'plink.log', 'plink.nosex', 'ref_id.list', 'id_train.txt']
...
def recalculate_snp_weights(X_train_1, id_train_1):
    ...
    try:
        subprocess.run(
            ['plink', '--bfile', 'train_genotypes', '--extract', 'final_snps', '--recodeA'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
def cross_validate_and_optimize_binary(model_class, param_space, X_train, y_train, snp_weights=None):
    if snp_weights is not None:
        model = BayesSearchCV(
            model_class(weights=snp_weights, sigma_a2=sigma_a2_estimated_GWKBR, sigma_e2=sigma_e2_estimated_GWKBR),
            param_space, n_iter=50, cv=5, n_jobs=-1, random_state=42, scoring=auroc_scorer
        )
    else:
        model = BayesSearchCV(
            model_class(sigma_a2=sigma_a2_estimated_KBR, sigma_e2=sigma_e2_estimated_KBR),
...
id_train = np.loadtxt('./example_data/X_train.txt', usecols=0)
...
# Calculate snp weights
snp_weights = calculate_snp_weights(X_train, id_train)
...
if trait_type == 'binary':
    ...
    param_space_KBR = {"gamma": Real(1e-8, 1e-3, prior="log-uniform"), "lambda_": Real(0.1, 10, prior="log-uniform")}
    best_params_KBR = cross_validate_and_optimize_binary(KBR_Model, param_space_KBR, X_train, y_train)
           
    print("Optimizing GWKBR model...")
    param_space_GWKBR = {"gamma": Real(1e-8, 1e-3, prior="log-uniform"), "lambda_": Real(0.1, 10, prior="log-uniform")}
...
def cross_validate_model_selection(model_class, best_params, X_train, y_train, id_train, model_name, snp_weights=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(rkf.split(X_train)):
        ...
        if model_name == "GWKBR":
            ...
            loc = {snp: i for i, snp in enumerate(all_snps.tolist())}
            snp_indices = [loc[s] for s in final_snps if s in loc]
            if len(snp_indices) != len(final_snps):
                missing = set(final_snps) - set(loc.keys())
                for ms in list(missing)[:5]:
                    print(f"Warning: SNP {ms} not found in all_snps.")
            X_train_1 = X_train_1[:, snp_indices]
            X_val_1   = X_val_1[:, snp_indices]
           
            # Delete the temporary files
            files_to_remove = ['id_train_temp.txt', 'train_id.list', 'train_genotypes.bed', 'train_genotypes.bim', 'train_genotypes.fam', 'gwas.result', 'final_snps', 'snp_weight.out', 'plink.raw', 'plink.log', 'plink.nosex']
            for file in files_to_remove:
                try:
                    os.remove(file)
                except FileNotFoundError:
